...
<div>
  ...
  <div class="row">
    <div
      ...
      @click="swap(index)"
    >
      <img :src="require(`../assets/${puzzleId}/${s}`)" alt="s.title" />
    </div>
  </div>
</div>
</template>
...
export default {
  ...
  computed: {
    isWinning () {
      correctPuzzleArray.forEach((puzzle, index) => {
        if (puzzle !== this.shuffledPuzzleArray[index]) {
          return false
        }
      })
      return true
  ...
  methods: {
    ...
    recordSpeedRecords () {
      ...
      const stringfiedRecords = JSON.stringify(sortedRecords)
